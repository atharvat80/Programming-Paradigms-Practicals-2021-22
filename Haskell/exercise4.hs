...
-- The first entry in the list must be prime, then we recurse on the tail, 
-- filtering out all those elements which are divisible by our prime.
-- This is actually doing "trial division" rather than the true sieve of Eratosthenes.
...
-- mult :: [[Int]] -> [Int] -> [Int]
-- mult xs ys = [sum $ zipWith (*)  ws ys| ws <- xs]
